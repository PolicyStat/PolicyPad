...
//    log("making simple Xhr: "+[method, uri, async, params].join(", "));
  ...
  var postPath = function() {
    return "/comet/post?r="+randomVar()+"&v="+version+"&id="+id+"&seq="+lastReceivedSeqNumber;
  }
  ...
  //FIXME: validChannels and canUseSubdomains should not be hardcoded

  var validChannels = ['shortpolling', 'longpolling', 'streaming'];
  var canUseSubdomains = true;
  var activeChannels = {};
  ...
  function channelPath() {
    return "/comet/channel?v="+version+"&r="+randomVar()+"&id="+id;
  }
  ...
  function StreamingChannel() {
    ...
    this.connect = function() {
      ...
      if (canUseSubdomains) {
        //FIXME: var streamurl = "//"+randomVar()+".comet."+host+"/comet/channel?v=2"+"&r="+randomVar()+"&id="+id+"&channel=streaming&type=iframe&new=yes&create="+(socket.readyState == socket.OPEN ? "no" : "yes")+"&seq="+lastReceivedSeqNumber;
...
//          theStream.attr('src', streamurl);
  ...
  function iframePath(key) {
    return "//"+key+".comet."+host+"/comet/xhrXdFrame";
  }
